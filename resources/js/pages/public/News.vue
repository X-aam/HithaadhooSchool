...
const categories: { key: NewsCategory | 'all'; label: { en: string; dv: string } }[] = [
...
<template>
    ...
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <Reveal v-for="(article, i) in paged" :key="article.id" :delay="(i % 3) * 90">
                <Link :href="`/news/${article.slug}`" class="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                    <div class="aspect-[16/10] overflow-hidden">
                        <img :src="article.image" :alt="pick(article.title)" class="size-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" />
                    </div>
                    <div class="flex flex-1 flex-col p-5">
                        <div class="flex items-center gap-2 text-xs">
                            <span class="rounded-full bg-brand-muted px-2.5 py-0.5 font-medium text-brand">{{ t(categories.find((c) => c.key === article.category)!.label) }}</span>
                            <span class="text-muted-foreground">{{ date(article.date) }}</span>
                        </div>
                        <h2 class="mt-3 text-lg font-semibold text-foreground group-hover:text-brand" dir="auto">{{ pick(article.title) }}</h2>
                        <p class="mt-2 line-clamp-3 text-sm text-muted-foreground" dir="auto">{{ pick(article.excerpt) }}</p>
        ...
        <nav v-if="totalPages > 1" class="mt-10 flex items-center justify-center gap-1.5" :aria-label="t(messages.common.menu)">
            <button
                type="button"
                ...
                @click="goTo(page - 1)"
            >
                <ArrowRight class="size-4 rotate-180" :class="isRtl ? 'rotate-0' : ''" />
            </button>
            <button
                ...
                :class="p === page ? 'border-brand bg-brand text-brand-foreground' : 'border-border text-foreground/70 hover:border-brand/40 hover:text-brand'"
                @click="goTo(p)"
            >
                {{ p }}
            </button>
